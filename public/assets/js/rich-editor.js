(function () {
    ...
    function createModal(title, bodyHtml) {
        ...
        modal.className = 'ck ck-modal';
        modal.innerHTML = `
            <div class="ck-modal__backdrop"></div>
            <div class="ck-modal__dialog ck-modal__dialog--wide">
                <div class="ck-modal__header">
                    <h3 class="ck-heading">${title}</h3>
                    <button type="button" class="ck ck-button ck-button_with-text ck-modal__close">×</button>
                </div>
        ...
        modal.querySelector('.ck-modal__close').addEventListener('click', close);
    ...
    if (!CKEDITOR_NS || !CKEDITOR_NS.ClassicEditor) {
        return;
    }

    const BasePlugin = CKEDITOR_NS.Plugin;
    const ButtonView = CKEDITOR_NS.ui.button.ButtonView;
    const dropdownUtils = CKEDITOR_NS.ui.dropdownUtils;
    const Collection = CKEDITOR_NS.utils.Collection;
        ...
        init() {
            ...
            const addUpload = () => {
                ...
                input.addEventListener('change', (event) => {
                    ...
                    fetch(settings.fileUploadUrl, { method: 'POST', body: formData })
                        .then((response) => response.json())
                        .then((payload) => {
                            if (payload?.url) {
                                insertHtml(editor, `<img src="${payload.url}" alt="" />`);
                            }
                        });
                });
                input.click();
            ...
            const addFromUrl = () => {
                ...
                if (url) {
                    insertHtml(editor, `<img src="${url}" alt="" />`);
                }
            };
    ...
    class TagsPlugin extends BasePlugin {
        ...
        init() {
            ...
            editor.ui.componentFactory.add('Tags', (locale) => {
                ...
                const definitions = new Collection(tags.map((tag) => {
    ...
    function initEditors() {
        ...
        const toolbarItems = [
            ...
            'link', 'blockQuote', 'uploadImage', 'insertImage', 'mediaEmbed', 'insertTable', 'horizontalLine', 'pageBreak',
        ...
        const removePlugins = [
            ...
            'FormatPainter', 'TableOfContents', 'PasteFromOfficeEnhanced', 'CaseChange', 'AIAssistance'
        ];
