...
class QueueManager
{
    ...
    {
        if ($this->sesService === null) $this->sesService = new SESService();
        return $this->sesService;
    }
    
    protected function getBounceService(): BounceNotificationService
    {
        if ($this->bounceService === null) $this->bounceService = new BounceNotificationService();
        ...
        foreach ($this->pendingSendsGenerator($batchSize, $now) as $send) {
            try {
                if (($send['message_status'] ?? '') === 'scheduled') {
                    $this->messageModel->update($send['message_id'], ['status' => 'sending']);
                }
    ...
    protected function sendEmail(array $send): array
    {
        ...
        $contact = $this->contactModel->select('id, email, name, nickname, is_active, opted_out, bounced')
        ->find($send['contact_id']);
        if (!$contact) return ['success' => false, 'error' => 'Contact not found'];
        
        if (!$contact['is_active'] || $contact['opted_out'] || $contact['bounced']) {
            $this->sendModel->update($send['id'], ['status' => 'cancelled']);
            return ['success' => false, 'status' => 'skipped', 'error' => 'Contact inactive'];
        }
        ...
        $result = $this->getSESService()->sendEmail(
            ...
            tags: [['Name' => 'message_id', 'Value' => (string) $message['id']]]
            );
        
        if ($result['success']) {
            $this->sendModel->update($send['id'], ['status' => 'sent', 'sent_at' => $this->now()]);
            $this->messageModel->increment($message['id'], 'total_sent');
            return ['success' => true];
        }
        
        $this->sendModel->update($send['id'], ['status' => 'failed']);
    ...
    protected function processSingleResend(array $rule, $db): void
    {
        ...
        if ($existing) {
            $db->table('resend_rules')->where('id', $rule['id'])->update(['status' => 'completed']);
            return;
        }
    ...
    {
        $htmlContent = str_replace(['{{nome}}', '{{apelido}}', '{{email}}'], [$contact['name'], $contact['nickname'], $contact['email']], $htmlContent);
